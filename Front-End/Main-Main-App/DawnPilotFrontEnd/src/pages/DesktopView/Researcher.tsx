...
function ResearcherView() {
  ...
  useEffect(() => {
    ...
    ws.onerror = (error) => console.error("🔴 [Researcher] AI WebSocket Error:", error);
    ws.onclose = () => {
  ...
  useEffect(() => {
    ...
    const handleCameraUpdate = (data: any) => {
      if (data.clientId && data.clientId !== socket.id) setMobileId(data.clientId);
    };
  ...
  useEffect(() => {
    loadWorld().catch(err => console.error('Researcher - Failed to load world:', err));
  }, [loadWorld]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setElapsedTime(Date.now() - vault.startTime!), 1000);
  ...
  return (
    <div style={{ display: 'flex', width: '100vw', height: '100vh', background: '#1a1a1a', color: '#eee', fontFamily: 'Segoe UI, Roboto, sans-serif' }}>
      ...
      <div style={{ width: '320px', background: '#222', borderRight: '1px solid #444', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
        <div style={{ padding: '20px', borderBottom: '1px solid #444', background: '#2d2d2d' }}>
          ...
          <div style={{ fontSize: '12px', color: '#aaa', marginTop: '5px' }}>
             Laptop: {isConnected ? '🟢' : '🔴'} | Mobile: {mobileId ? '🟢' : '🔴'} | AI: {aiWebSocket?.readyState === WebSocket.OPEN ? '🟢' : '🔴'}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
          <div style={{ marginBottom: '24px', background: '#333', padding: '15px', borderRadius: '8px' }}>
            ...
            <div style={{ marginBottom: '10px' }}>
              ...
              <input type="text" value={subjectId} onChange={(e) => setSubjectId(e.target.value)} disabled={vault.isRecording} style={{ width: '100%', padding: '5px', background: '#222', border: '1px solid #444', color: 'white' }} />
            </div>
            <div style={{ marginBottom: '10px' }}>
              <label style={{ fontSize: '11px', color: '#aaa' }}>Vision Mode</label>
              <select value={visionMode} onChange={(e) => setVisionMode(e.target.value)} disabled={vault.isRecording} style={{ width: '100%', padding: '5px', background: '#222', border: '1px solid #444', color: 'white' }}>
            ...
            {!vault.isRecording ? (
              <button onClick={handleStartExperiment} disabled={!mobileId} style={{ width: '100%', padding: '10px', background: mobileId ? '#4CAF50' : '#555', color: 'white', border: 'none', borderRadius: '4px', cursor: mobileId ? 'pointer' : 'not-allowed', fontWeight: 'bold' }}>{vault.isLoading ? 'Starting...' : 'Start Recording'}</button>
            ) : (
              <button onClick={handleStopExperiment} style={{ width: '100%', padding: '10px', background: '#f44336', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>{vault.isLoading ? 'Stopping...' : 'Stop Recording'}</button>
            )}
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ fontSize: '32px', fontWeight: 'bold', fontFamily: 'monospace' }}>{formatTime(elapsedTime)}</div>
          </div>
          ...
          <div style={{ marginBottom: '24px', background: '#333', borderRadius: '8px', padding: '15px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: '20px', fontWeight: 'bold', color: collisionCount > 0 ? '#ff6b6b' : '#fff' }}>{collisionCount}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, fontSize: '12px', color: '#ccc' }}>
              ...
              {collisionLog.map((log, idx) => <li key={idx} style={{ marginBottom: '6px', borderBottom: '1px solid #333', paddingBottom: '4px' }}>{log}</li>)}
            </ul>
          </div>

          <div style={{ marginTop: '24px' }}>
            <button style={{ backgroundColor: '#00ff88', color: '#000', fontWeight: 'bold', border: 'none', padding: '8px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', width: '100%' }} onClick={handleOpenLoadDialog} disabled={saveLoadLoading || vault.isRecording}>📂 Load Scenario</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, position: 'relative', background: 'black', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ width: '100%', maxWidth: '177.78vh', aspectRatio: '16/9', position: 'relative' }}>
        ...
        {showLoadDialog && <ScenarioLoadDialog scenarios={savedScenarios} onLoad={handleLoadScenario} onCancel={() => setShowLoadDialog(false)} onDelete={(id) => deleteScenarioAPI(id).then(() => listScenarios().then(setSavedScenarios))} loading={saveLoadLoading} />}
      </div>
    </div>
  );
}
